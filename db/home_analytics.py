    ...
    try:
        QUERY = (
            ...
            SELECT pvt.*, DENSE_RANK() OVER (PARTITION BY 
                pvt.subject_id, pvt.hadm_id,pvt.stay_id,pvt.label ORDER BY pvt.charttime) as drank
            FROM pvt
            )
            ...
            FROM ranked r
            WHERE r.drank = 1
            GROUP BY r.subject_id, r.hadm_id, r.stay_id, r.mort_icu, r.mort_hosp, r.drank
            ORDER BY r.subject_id, r.hadm_id, r.stay_id, r.mort_icu, r.mort_hosp, r.drank;
            """
        )
